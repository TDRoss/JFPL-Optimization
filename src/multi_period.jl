...
function solve_multi_period_fpl(merged_data, team_data, type_data, next_gw, initial_squad,itb, ft, horizon, objective ="regular", decay_base=0.84)
    ...
    optimize!(model)
	# picks = DataFrame(gameweek = [], name = [], pos = [], team = [], price = [], xP = [], lineup = [], captain = [], vicecap = [], transfer_in = [], transfer_out = [])
...
picks, total_xp, summary_of_actions =  solve_multi_period_fpl(merged_data, team_data, type_data, next_gw, initial_squad, itb, 1, 4, "decay")
