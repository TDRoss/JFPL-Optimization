...
# ╔═╡ 9738f3f8-45e0-11ee-05e6-677200a5784c
using CSV, DataFrames, JuMP, SCIP, PlutoUI, HTTP, JSON

# ╔═╡ ee972ad9-5715-4631-a5e0-1e169eea2b87
begin
	#Data
...
# ╔═╡ 99285fbe-5ebd-48fc-b5a9-ce2c3a1a53f7
# ╠═╡ disabled = true
#=╠═╡

  ╠═╡ =#

# ╔═╡ 1a36c60a-2cc3-42a7-beba-cfe8f7ce53bc
begin
	model = Model(SCIP.Optimizer)
	playid = merged_data[:,:id]
...
# ╔═╡ e3b12f85-71f4-416b-a888-b1cb11f6c566
begin
...
# ╔═╡ a56afa62-fcac-4951-8cca-20eb6fde8210
...
# ╔═╡ 3f7460eb-02f4-4743-aa2c-c1124a675047
begin
	budget = 100;
	price = sum(merged_data[:,:now_cost].*squad./10);
end

# ╔═╡ f245f74c-6534-4655-ba9e-a24190bd26a8
@constraint(model,price<=budget)

# ╔═╡ 63bc6b13-e77d-4e0c-955b-1ed7549581ba
for t in teams
@constraint(model, sum(squad.*(merged_data[:,"name"] .== t)) <=3)
end

# ╔═╡ 1922206d-7e0c-42dd-ac77-24b60cf92794
total_points = sum(merged_data[:,"$(next_gw)_Pts"] .* (lineup .+ captain .+ 0.1.*vicecap));

# ╔═╡ f4a7e984-352e-4bde-96cb-7e072c3c5693
@objective(model,Max,total_points);

# ╔═╡ 1d807a07-655b-48f1-9a4c-7188ad80b4a1
optimize!(model)

# ╔═╡ 0583f983-c002-40dd-a24e-c5fe7a8640a4
objective_value(model)

# ╔═╡ 36130589-5b9f-4c0e-8cb2-3114f45d26c8
begin
	optlineup = (1:nplayers)[Bool.(value.(lineup))];
	optsquad = (1:nplayers)[Bool.(value.(squad))];
	optcap = (1:nplayers)[Bool.(value.(captain))];
	optvicecap = (1:nplayers)[Bool.(value.(vicecap))];
end

# ╔═╡ 468bab68-5e4a-4532-94ca-c04bdc0d72ae
begin
...
# ╔═╡ 3f39118c-4c00-4c51-839b-5f410a189b59
picks

# ╔═╡ c7e5124e-0aad-485f-a63f-48b0b18d39e6
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "2b142047c093e02caa126791d353b04e72ba62b9"
...
[[deps.SCIP]]
deps = ["Ipopt_jll", "Libdl", "LinearAlgebra", "MathOptInterface", "SCIP_PaPILO_jll", "SCIP_jll"]
git-tree-sha1 = "3b43e52fa7fd045a8b9ee20a3ec5a38716fc5c77"
...
[[deps.oneTBB_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl", "Pkg"]
git-tree-sha1 = "cefc4c476923c22a025d1124dfd10c5781160cea"
